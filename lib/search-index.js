...
function pinyinInitial(ch) {
	...
	let prev = null;
	for (const [letter, bound] of PINYIN_BOUNDS) {
		if (collator.compare(ch, bound) < 0) return prev;
		prev = letter;
	}
	return "z";
}
...
export function pinyinAbbr(name) {
	let out = "";
	for (const ch of name) {
		...
		const initial = pinyinInitial(ch);
...
function scoreRow(kw, row) {
	...
	if (ascii) {
		if (row.abbr === kw) return 96 + base;
		if (row.abbr.startsWith(kw)) return 86 - Math.min(row.abbr.length - kw.length, 9) + base;
		if (row.lower.startsWith(kw)) return 82 + base; // ASCII inside the name, e.g. "TCL科技"
		if (row.abbr.includes(kw)) return 60 + base;
		if (row.lower.includes(kw)) return 55 + base;
		return -1;
	}
