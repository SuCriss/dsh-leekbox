...
import { fetchText, fetchJson, feedError } from "./fetch-utils.js";
...
async function fetchUniverse(fs) {
	...
	if (first === null) throw feedError("全市场股票清单构建失败：上游首页取不到", "instrument index: first page failed");
...
async function buildIndex() {
	...
	if (rows.length < 100) throw feedError("全市场股票清单构建失败：返回的股票数量异常偏少", "instrument index: suspiciously small snapshot");
