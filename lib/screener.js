...
//
// Pipeline: universe snapshot (Sina getHQNodeData) → basic filters → per-stock
// daily K-lines (Tencent fqkline, 120 bars) → indicator computation → signal
// matching + weighted score → sorted results.
...
async function buildUniverse(node, limit) {
	...
	outer: while (page <= maxPages) {
		...
		const results = await Promise.all(
			chunk.map((p) =>
				fetchJson(`${SINA_HQ}?page=${p}&num=100&sort=amount&asc=0&node=${node}`)
			)
		);
...
async function getKline(code) {
	...
	const url = `${TENCENT_FQKLINE}?param=${encodeURIComponent(`${code},day,,,120,qfq`)}`;
