...
export async function runScreener(params) {
	...
	try {
		...
		const strategyKeys = Array.isArray(params.strategies) ? params.strategies.filter((k) => STRATEGY_META[k]) : [];
		const minStrategyHits = params.minStrategyHits ?? 2;
		const rows = [];
		for (const r of candidates) {
			...
			if (mode === "multi") {
				// Multi-strategy intersection: the stock must hit enough preset
				// strategies simultaneously. Score is still computed and shown for
				// ranking context, but the selection gate is the strategy count.
				if (strategyKeys.length > 0) {
					const hits = strategyHits(sig, strategyKeys);
					if (hits.length < minStrategyHits) continue;
					rows.push({
						code: r.code,
						name: r.name,
						symbol: code,
						...
						score: scoreOf(sig),
						strategyCount: hits.length,
						strategies: hits.map((x) => x.label),
