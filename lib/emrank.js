...
async function fetchEastmoneyRankPage({ node = "hs_a", sort = "changepercent", order = "desc", page = 1, size = 30 }) {
	if (clistThrottled()) throw new Error("eastmoney clist throttled (cooldown)");
	...
	if (!Array.isArray(diff)) {
		markClistDown();
		throw lastError instanceof Error ? lastError : new Error("eastmoney clist unavailable");
	}
	markClistUp();
...
export async function fetchEmRankPage(opts = {}) {
	...
	try {
		...
		const em = emError instanceof Error ? emError.message : String(emError);
		if (!sinaRankSupports(node, sort)) {
			throw new Error(`rank feed unavailable — eastmoney: ${em}; sina: no fallback for node=${node} sort=${sort}`);
		}
		try {
			return await fetchSinaRankPage(opts);
		} catch (sinaError) {
			const sina = sinaError instanceof Error ? sinaError.message : String(sinaError);
			throw new Error(`rank feed unavailable — eastmoney: ${em}; sina: ${sina}`);
		}
	}
}
...
export async function fetchEmSectorPage(opts = {}) {
	...
	try {
		...
		if (!sinaSectorSupports(type, sort)) {
			throw new Error(`sector feed unavailable — eastmoney: ${em}; sina: no fallback for type=${type} sort=${sort}`);
		}
		try {
			return await fetchSinaSectorPage(opts);
		} catch (sinaError) {
			const sina = sinaError instanceof Error ? sinaError.message : String(sinaError);
			throw new Error(`sector feed unavailable — eastmoney: ${em}; sina: ${sina}`);
		}
	}
}
...
async function fetchEastmoneySectorPage({ type = "industry", sort = "f3", order = "desc", page = 1, size = 30 } = {}) {
	...
	if (!Array.isArray(diff)) {
		markClistDown();
		throw lastError instanceof Error ? lastError : new Error("eastmoney sector unavailable");
	}
	markClistUp();
	const rows = diff.map((r) => ({
		code: String(r.f12 ?? ""),
...
export async function fetchEmBreadth() {
	...
	if (breadthCache.failedAt && Date.now() - breadthCache.failedAt < BREADTH_FAIL_BACKOFF) {
		throw new Error("eastmoney breadth unavailable (recent failure)");
	}
	...
	if (total === 0) {
		breadthCache.failedAt = Date.now();
		throw new Error("eastmoney breadth: index snapshot carried no 涨跌家数");
	}
