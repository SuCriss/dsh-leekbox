 ...
 * @param {object} opts
 * @param {string} [opts.node] hs_a | sh_a | sz_a | cyb | kcb | main | non_main
...
/** 首页两轮全失败后的快速失败窗口:期间直接抛错,不再重复 20s+ 的超时等待。 */
...
export async function fetchEmBreadth() {
	...
	// Fetch the remaining pages in chunks of 14, tolerating page failures.
	...
	for (let start = 2; start <= pages; start += 14) {
		const chunk = [];
		for (let p = start; p < start + 14 && p <= pages; p++) chunk.push(p);
