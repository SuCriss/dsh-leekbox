...
`;
		...
		function MarketTab({ onOpen, watchCodes }) {
			...
			const loadRank = useCallback(() => {
				...
				setLoading(true);
				api(API.rank + `?sort=${choice.key}&order=${choice.order}&page=1&size=30`)
					.then((data) => {
						setRows(data.rows ?? []);
						setError("");
					})
			...
			useInterval(loadRank, 60000, [rankKey]);
			return h(
				"div",
				null,
				...
				hits.length > 0
					...
					: null,
				h("div", { className: "lkb-chipRow" }, RANK_CHOICES.map((c, i) => h("button", { key: c.label, className: "lkb-chip", "data-active": rankKey === i ? "true" : "false", onClick: () => setRankKey(i) }, c.label))),
				error === "" ? null : h("div", { className: "lkb-error" }, error),
				h(
					"table",
					null,
				...
				loading && rows.length === 0 ? h("div", { className: "lkb-empty" }, "加载中…") : null
			);
		}
		//#endregion
