...
.lkb-stickyTop{position:sticky;top:-14px;z-index:8;background:var(--dsw-alias-bg-layer-1,#f7f8fa);margin:-14px -18px 10px;padding:10px 18px 4px;display:flex;flex-direction:column;gap:6px;box-shadow:0 8px 14px -12px #00000059}
...
`;
		...
		function MarketTab({ onOpen, watchCodes }) {
			...
			const loadRank = useCallback(() => {
				...
				api(API.rank + `?sort=${choice.key}&order=${choice.order}&page=${page}&size=${pageSize}`)
					.then((data) => {
			...
			return h(
				...
				hits.length > 0
					...
					: null,
				sentiment !== null && mode === "stocks"
					? h(
							"div",
							{ className: "lkb-chipRow", style: { justifyContent: "flex-start", gap: 14, padding: "2px 2px 0" } },
							h("span", { className: "lkb-status" }, `涨停 ${sentiment.limitUp ?? "—"}`),
							h("span", { className: "lkb-status" }, `跌停 ${sentiment.limitDown ?? "—"}`),
							h("span", { className: "lkb-status" }, `全市场 ${sentiment.marketTotal ?? "—"} 只`)
						)
					: null,
				h("div", { className: "lkb-chipRow" }, BOARD_MODES.map((m) => h("button", { key: m.key, className: "lkb-chip", "data-active": mode === m.key ? "true" : "false", onClick: () => setMode(m.key) }, m.label))),
				mode === "stocks"
									...
									h(
										...
										lhLoading
											...
											: lhRows.map((r) =>
													h(
														"tr",
														{ key: r.code + r.reason, onClick: () => onOpen(r.code, r.name) },
														h("td", null, h("span", { className: "lkb-name" }, r.name), h("span", { className: "lkb-code" }, r.code)),
				...
				mode === "stocks" && total > 0
					...
					: mode === "sector" && sectorTotal > 0
						...
						: mode === "longhu" && lhTotal > 0
							? h("div", { className: "lkb-pager" }, h("span", { className: "lkb-status" }, `今日上榜 ${lhTotal} 只`))
							: null
			);
		}
		//#endregion
						...
						shown.map((it) => {
							...
							return h(
								...
								h(
									...
									h(StarButton, { code: it.code, on: true })
								)
							);
						})
					)
