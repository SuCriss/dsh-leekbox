...
`;
		...
		function srLevels(klines, idx) {
			...
			// 最终兜底:区间极值(保证 压力>=收盘>=支撑)
			if (res === null || res <= cur.close) res = Math.max(...win.map((b) => b.high));
			if (sup === null || sup >= cur.close) sup = Math.min(...win.map((b) => b.low));
			return { res, sup };
		}

		// 全序列逐日滚动 S/R,供画线使用;每晚 2 根K线才开始有摆动点,更早的用枢轴点/极值回退
		function srSeries(klines) {
			const res = new Array(klines.length).fill(null);
			const sup = new Array(klines.length).fill(null);
			for (let i = 0; i < klines.length; i++) {
				const r = srLevels(klines, i);
				res[i] = r.res;
				sup[i] = r.sup;
			}
			return { res, sup };
			...
			return h(
				...
				h(
					...
					h(
						...
						h("polyline", { key: "srRes", points: srS.res.map((v, i) => `${xc(i).toFixed(1)},${py(v).toFixed(1)}`).join(" "), fill: "none", stroke: UP_COLOR, strokeWidth: 1.1, vectorEffect: "non-scaling-stroke", opacity: 0.9 }),
						h("polyline", { key: "srSup", points: srS.sup.map((v, i) => `${xc(i).toFixed(1)},${py(v).toFixed(1)}`).join(" "), fill: "none", stroke: DOWN_COLOR, strokeWidth: 1.1, vectorEffect: "non-scaling-stroke", opacity: 0.9 }),
						h("line", {
							x1: 0,
							x2: W,
