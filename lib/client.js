...
`;
		...
		//#region shared bits
		function StarButton({ code, on, confirmText }) {
			return h(
				"button",
				{
					className: "lkb-star",
					...
					? h(
							...
							hits.map((s) =>
								h(
									...
									h("span", { style: { marginLeft: "auto" } }, h(StarButton, { code: s.code, on: watchSet.has(s.code), confirmText: `确定将 ${s.name}（${s.code}）移出自选吗？` }))
								)
							)
						)
					: null,
							...
							h(
								...
								rows.map((r) =>
									h(
										...
										h("td", null, h(StarButton, { code: r.code, on: watchSet.has(r.code), confirmText: `确定将 ${r.name}（${r.code}）移出自选吗？` }))
									)
								)
							)
						)
						...
						shown.map((it) => {
							...
							return h(
								...
								h(
									...
									h(StarButton, { code: it.code, on: true, confirmText: `确定将 ${it.name}（${it.code}）移出自选吗？` })
								)
							);
						})
					)
