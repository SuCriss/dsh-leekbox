...
.lkb_card tbody tr{cursor:pointer;transition:background .1s}
...
/* ---- 市场情绪条 + 报价板单元格(行情页) ---- */
.lkb-senti{flex:1;min-width:0;display:flex;align-items:stretch;flex-wrap:wrap;background:var(--dsw-alias-bg-base,#fff);border:1px solid var(--dsw-alias-border-l1,#e8e9ed);border-radius:12px;overflow:hidden}
.lkb-breadth{flex:1;min-width:150px;display:flex;flex-direction:column;justify-content:center;gap:5px;padding:7px 14px;border-right:1px solid var(--dsw-alias-border-l1,#f0f1f4)}
.lkb-breadthNums{display:flex;justify-content:space-between;align-items:baseline;gap:8px;font-size:11.5px;color:var(--dsw-alias-label-tertiary,#a2a7b3);font-variant-numeric:tabular-nums;white-space:nowrap}
.lkb-breadthNums b{font-size:12.5px;font-weight:700}
.lkb-breadthBar{height:6px;border-radius:99px;background:#0f9d6e2e;display:flex;overflow:hidden}
.lkb-breadthUp{background:#e03131;height:100%;border-radius:99px 0 0 99px;transition:width .4s ease}
.lkb-sentiCell{display:flex;flex-direction:column;justify-content:center;gap:1px;padding:6px 13px;border-right:1px solid var(--dsw-alias-border-l1,#f0f1f4);white-space:nowrap}
.lkb-sentiCell:last-child{border-right:0}
.lkb-sentiLabel{font-size:10.5px;color:var(--dsw-alias-label-tertiary,#a2a7b3);letter-spacing:.04em}
.lkb-sentiValue{font-size:16px;font-weight:700;line-height:1.15;font-variant-numeric:tabular-nums;color:var(--dsw-alias-label-primary,#1c1e26)}
.lkb-pct{display:inline-block;min-width:58px;text-align:center;font-weight:600;font-size:12px;padding:2px 8px;border-radius:7px;font-variant-numeric:tabular-nums}
...
body[data-ds-dark-theme] .lkb-senti{background:#3a3a3c;border-color:#ffffff14}
body[data-ds-dark-theme] .lkb-breadth,body[data-ds-dark-theme] .lkb-sentiCell{border-color:#ffffff0f}
body[data-ds-dark-theme] .lkb-breadthBar{background:#0f9d6e38}
...
.lkb-chip{cursor:pointer;background:var(--dsw-alias-bg-layer-1,#eef0f3);border:1px solid var(--dsw-alias-border-l1,#e3e5e9);color:var(--dsw-alias-label-secondary,#5f6672);border-radius:99px;padding:4px 12px;font-size:12px;white-space:nowrap;font-variant-numeric:tabular-nums;transition:border-color .12s, background .12s, color .12s}
...
`;
		...
		/** 上涨家数占(上涨+下跌)的百分比,用于市场宽度条;数据缺失时返回 50(中性)。 */
		function breadthPct(s) {
			const has = (v) => v !== null && v !== void 0 && String(v).trim() !== "" && Number.isFinite(Number(v));
			if (!has(s?.up) || !has(s?.down)) return 50;
			const up = Number(s.up);
			const down = Number(s.down);
			if (up + down <= 0) return 50;
			return Math.min(100, Math.max(0, (up / (up + down)) * 100));
		}
			...
			return h(
				...
				h(
					...
					h(
						"div",
						{ className: "lkb-senti" },
						// 涨跌家数占比条:一秒读出市场强弱,红涨绿跌
						h(
							"div",
							{ className: "lkb-breadth", title: `上涨 ${sentiment?.up ?? "—"} 家 / 下跌 ${sentiment?.down ?? "—"} 家` },
							h(
								"div",
								{ className: "lkb-breadthNums" },
								h("span", null, "上涨 ", h("b", { className: "lkb-up" }, sentiment?.up ?? "—")),
								h("span", null, "下跌 ", h("b", { className: "lkb-down" }, sentiment?.down ?? "—"))
							),
							h(
								"div",
								{ className: "lkb-breadthBar" },
								h("div", {
									className: "lkb-breadthUp",
									style: { width: breadthPct(sentiment) + "%" },
								})
							)
						),
						h("div", { className: "lkb-sentiCell" }, h("span", { className: "lkb-sentiLabel" }, "涨停"), h("span", { className: "lkb-sentiValue lkb-up" }, sentiment?.limitUp ?? "—")),
						h("div", { className: "lkb-sentiCell" }, h("span", { className: "lkb-sentiLabel" }, "跌停"), h("span", { className: "lkb-sentiValue lkb-down" }, sentiment?.limitDown ?? "—")),
						h("div", { className: "lkb-sentiCell" }, h("span", { className: "lkb-sentiLabel" }, "炸板"), h("span", { className: "lkb-sentiValue" }, sentiment?.broken ?? "—")),
						h("div", { className: "lkb-sentiCell" }, h("span", { className: "lkb-sentiLabel" }, "全市场"), h("span", { className: "lkb-sentiValue" }, sentiment?.marketTotal ?? "—"))
					),
					h(
						"div",
						{ className: "lkb-seg" },
							...
							h(
								...
								rows.map((r, i) =>
									h(
										...
										h("td", { className: "lkb-code" }, String((page - 1) * pageSize + i + 1)),
