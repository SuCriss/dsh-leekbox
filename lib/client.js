...
/* ---- sortable headers ---- */
.lkb-thSort{cursor:pointer;user-select:none;white-space:nowrap}
.lkb-thSort:hover{color:#4c6ef5}
.lkb-arrow{font-size:9px;margin-left:3px;color:#4c6ef5}
body[data-ds-dark-theme] .lkb-thSort:hover{color:#91a7ff}
body[data-ds-dark-theme] .lkb-arrow{color:#91a7ff}
`;
		...
		function normCode(raw) {
			...
			if (!/^\d{6}$/.test(d)) return c;
			if (/^[695]/.test(d)) return "sh" + d;
		...
		function StarButton({ code, on }) {
			return h(
				"button",
				{
					...
					onClick: (e) => {
						e.stopPropagation();
						api(API.watchlist + (on ? "/remove" : "/add"), { method: "POST", body: { code } })
							.then(() => {})
							.catch(() => {});
					},
				},
			...
			return h(
				...
				h(
					...
					indices.map((q) =>
						h(
							"div",
							{ className: "lkb-indexCard", key: q.code, onClick: () => onOpen(q.code, q.name) },
							h(
								"div",
		...
		/** Sortable table columns (key must be accepted by the rank route). */
		const SORT_COLS = [
			{ key: "price", label: "现价" },
		...
		function MarketTab({ onOpen, watchCodes }) {
			...
			const [sort, setSort] = useState(null); // header sort key; null = follow board preset
			...
			const loadRank = useCallback(() => {
				...
				const s = sort ?? choice.key;
				const o = sort === null ? choice.order : order;
				setLoading(true);
				// Keep showing the previous rows while the request is in flight;
				// only replace them once fresh data arrives (no table clearing).
				api(API.rank + `?sort=${s}&order=${o}&page=${page}&size=${pageSize}`)
					.then((data) => {
			...
			}, [rankKey, page, pageSize, sort, order]);
			const onSort = (key) => {
			...
			useInterval(loadRank, 60000);
			const activeSort = sort ?? RANK_CHOICES[rankKey].key;
			const activeOrder = sort === null ? RANK_CHOICES[rankKey].order : order;
			return h(
				"div",
				null,
				h("div", { className: "lkb-searchRow" }, h("input", { className: "lkb-input", placeholder: "搜索股票：代码 / 名称 / 拼音，如 600519、茅台、GZMT", value: kw, onChange: (e) => setKw(e.target.value) })),
				hits.length > 0
					? h(
							"div",
							...
							hits.map((s) =>
								h(
									...
									h("span", { style: { marginLeft: "auto" } }, h(StarButton, { code: s.code, on: watchSet.has(s.code) }))
								)
							)
						)
					: null,
				h("div", { className: "lkb-chipRow" }, RANK_CHOICES.map((c, i) => h("button", { key: c.label, className: "lkb-chip", "data-active": rankKey === i ? "true" : "false", onClick: () => { setRankKey(i); setPage(1); setSort(null); setOrder("desc"); } }, c.label))),
				loading
					? h(
							"div",
				...
				h(
					...
					h(
						...
						h(
							...
							...SORT_COLS.map((c) =>
								h(
									"th",
									{ className: "lkb-thSort", title: "点击排序", onClick: () => onSort(c.key) },
									c.label,
									activeSort === c.key ? h("span", { className: "lkb-arrow" }, activeOrder === "desc" ? "▼" : "▲") : null
								)
							),
							h("th", null, "")
						)
					...
					h(
						...
						rows.map((r) =>
							h(
								"tr",
								{ key: r.symbol, onClick: () => onOpen(r.code, r.name) },
								...
								h("td", null, h(StarButton, { code: r.code, on: watchSet.has(r.code) }))
							)
						)
					)
				),
		...
		function WatchlistTab({ onOpen, bump }) {
			...
			if (items.length === 0)
				return h("div", { className: "lkb-empty" }, "自选股为空。去「行情」搜索或从榜单点击 ☆ 添加。");
			return h(
				"table",
				null,
				h(
				...
				h(
					...
					items.map((it) => {
						...
						return h(
							"tr",
							{ key: it.code, onClick: () => onOpen(it.code, it.name) },
							h("td", null, h("span", { className: "lkb-name" }, it.name), h("span", { className: "lkb-code" }, it.code)),
							...
							h("td", null, h(StarButton, { code: it.code, on: true }))
						);
					})
				)
			);
		...
		function ScreenerTab({ onOpen, watchCodes }) {
			...
			const run = () => {
				...
				const params = {
					...
					require: SCREEN_SIGNALS.filter((s) => require[s.key]).map((s) => s.key),
				};
			...
			const pct =
				progress && progress.total > 0
					...
					: null;
			const selCount = Object.values(require).filter(Boolean).length;
			const resetAll = () => {
				setNode("hs_a");
				setUniverse(800);
			...
			return h(
				...
				h(
					...
					h(
						...
						h("div", { className: "lkb-scSub" }, "股票池 · 基础条件 · 技术信号 —— 实时日K计算指标，综合评分排序")
					),
					selCount > 0 ? h("span", { className: "lkb-sigCount", style: { fontSize: 11.5, padding: "3px 12px" }, "data-n": "true" }, `已选 ${selCount} 个信号`) : null
				),
				h(
					"div",
					...
					h(
						...
						h("span", { className: "lkb-status", style: { marginLeft: "auto" } }, "基于日K实时计算 MACD / KDJ / RSI / 均线 / 布林")
					),
					running && progress
						? h(
								"div",
				...
				ran && !running && error === "" && rows.length === 0
					? h(
							...
							"没有符合条件的股票",
							h("div", { style: { marginTop: 4, fontSize: 11.5 } }, "试试放宽价格 / 换手区间，或减少必选信号")
						)
					: null,
				rows.length > 0
					? h(
							...
							h(
								...
								h("span", null, `共 ${rows.length} 只匹配 · 按综合评分排序`),
										...
										h(
											...
											h(
												...
												h(
													...
													(r.signals ?? []).slice(0, 3).map((t) => h("span", { className: "lkb-signalTag", key: t }, t)),
													(r.signals ?? []).length > 3 ? h("span", { className: "lkb-signalMore", key: "__more" }, `+${r.signals.length - 3}`) : null
												)
											),
											h("td", null, h(StarButton, { code: r.code, on: watchSet.has(r.code) }))
										)
					...
					h(
						...
						h(
							...
							h(
								...
								["day", "week", "month", "m5", "m30"].map((p) =>
									h("button", { key: p, className: "lkb-period", "data-active": period === p ? "true" : "false", onClick: () => setPeriod(p) }, { day: "日K", week: "周K", month: "月K", m5: "5分", m30: "30分" }[p])
								)
							)
						),
