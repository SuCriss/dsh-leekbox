...
import { getInstrumentIndex, instrumentIndexStatus, searchInstruments, searchSmartbox, marketFromCode, tencentSymbol, TYPE_LABEL } from "./search-index.js";
		...
		},
		{
			...
			handler: async (req, res) => {
				...
				try {
					...
					if (klines === null) {
						writeJson(res, 502, { error: `没有取到 ${code} 的 K 线数据（周期 ${period}），请稍后重试` });
						return;
					}
			...
			handler: async (req, res) => {
				...
				try {
					...
					try {
						...
						console.error(`[leekbox] rank feed failed (node=${node} sort=${sort} page=${page}): ${reason}`);
						writeJson(res, 502, { error: userMessage(error), reason: errorDetail(error) || reason });
						return;
					}
			...
			handler: async (req, res) => {
				...
				try {
					...
					if (merged.length === 0 && settled.every((s) => s.status === "rejected")) {
						writeJson(res, 502, { error: "快讯源全部不可用，请稍后重试" });
						return;
					}
			...
			handler: async (req, res) => {
				...
				try {
					...
					if (!list.some((e) => e.code === normalized)) {
						...
						// Update group and/or name for existing entries.
						...
						if (changed) writeWatchlist(dshHome, list);
					}
					writeJson(res, 200, { watchlist: list });
				} catch (error) {
					fail(res, error);
			...
			handler: async (req, res) => {
				...
				try {
					...
					const params = {
						...
						mode: body.mode === "multi" ? "multi" : "standard",
						strategies: Array.isArray(body.strategies) ? body.strategies.filter((k) => typeof k === "string") : [],
						minStrategyHits: numParam(body.minStrategyHits) ?? 2,
					};
					const result = await runScreener(params);
					writeJson(res, 200, result);
				} catch (error) {
					if (error instanceof Error && (error.code === "screener_already_running" || /already running/i.test(error.message))) {
						writeJson(res, 409, { error: userMessage(error) });
						return;
					}
					// Client errors thrown by the screener's parameter validation
					// are 400s, not upstream failures.
					if (error instanceof Error && /^(未知|multi 模式)/.test(error.message)) {
