...
import { fetchEmRankPage, fetchEmDailyKline, fetchEmSectorPage, fetchEmLimitPools, fetchEmFflowKline, fetchEmLonghu } from "./emrank.js";
...
function normalizeCode(raw) {	let code = String(raw).trim().toLowerCase();
	...
	return [
		...
		},
		{
			...
			handler: async (req, res) => {
				if (!guard(req, res, "GET")) return;
				try {
					const pools = await fetchEmLimitPools();
					...
					writeJson(res, 200, { ok: true, limitUp: pools.limitUp, limitDown: pools.limitDown, marketTotal });
				} catch (error) {
					fail(res, error);
				}
			},
		...
		},
		{
			...
			handler: async (req, res) => {
				...
				try {
					...
					writeJson(res, 200, { date, page, size, total: data.total, rows: data.rows });
				} catch (error) {
					fail(res, error);
				}
			},
