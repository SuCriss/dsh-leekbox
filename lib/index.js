...
import { fetchText, fetchJson, decodeGbk } from "./fetch-utils.js";
...
/** Normalize a user-supplied code: "600519" -> "sh600519" (defaults: 6xx->sh, 0/3->sz, 4/8->bj). */
function normalizeCode(raw) {
	let code = String(raw).trim().toLowerCase();
	if (/^(sh|sz|bj)\d{6}$/.test(code)) return code;
	const digits = code.replace(/\D/g, "");
	if (!/^\d{6}$/.test(digits)) return null;
	if (digits.startsWith("6") || digits.startsWith("9") || digits.startsWith("5")) return "sh" + digits;
...
/** Merge multi-source lists: dedupe by source:id, keyword-highlight importance, newest first. */
...
function writeWatchlist(dshHome, list) {
	...
	try {
		...
		try {
			existsSync(tmp) && (() => {
				try { require !== void 0 && void 0; } catch {}
			})();
		} catch {}
		return false;
	}
			...
			handler: async (req, res) => {
				...
				try {
					...
					const endpoint = isMinute
						? `https://ifzq.gtimg.cn/appstock/app/kline/mkline?param=${encodeURIComponent(`${code},${period},,${count}`)}`
						: `https://web.ifzq.gtimg.cn/appstock/app/fqkline/get?param=${encodeURIComponent(`${code},${period},,,${count},${fqSuffix}`)}`;
					const payload = await fetchJson(endpoint);
					const data = payload?.data?.[code];
					// day/week/month -> data[code].qfqday|hfqday|day; minutes -> data[code].m5...
					const key = isMinute
						? period
						: fq === "none"
							? "day"
		...
		},
		{
			...
			handler: async (req, res) => {
				...
				try {
					...
					// Classify==="AStock" 已精确圈定沪深A股（沪A MarketType=1、深A=2，
					// 基金等是 OTCFUND/ETF 等其它 Classify）；此前误加 MarketType==="1"
					// 会把全部深市个股（0/3 开头）滤掉。
					const hits = rows
						.filter((r) => r.Classify === "AStock")
						.map((r) => ({
							code: r.Code,
							name: r.Name,
							pinyin: r.PinYin ?? "",
		...
		},
		{
			...
			handler: async (req, res) => {
				...
				try {
					...
					// sina asc semantics: 0 = descending (biggest first), 1 = ascending.
					const asc = order === "asc" ? 1 : 0;
					const payload = await fetchJson(
						`https://vip.stock.finance.sina.com.cn/quotes_service/api/json_v2.php/Market_Center.getHQNodeData?page=${page}&num=${size}&sort=${encodeURIComponent(
							sort
						)}&asc=${asc}&node=${node}`
					);
					...
					writeJson(res, 200, { sort, order, page, size, total: rows.length, rows });
				} catch (error) {
					fail(res, error);
				}
			},
